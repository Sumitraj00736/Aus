      ...
      .then((data) => {
        ...
        if (!data || data.length === 0) {
          setPosts([
            ...
            {
              _id: "3",
              title: "The Germiest Spots in Your Home (and How Pros...",
              slug: "germs",
              coverImage:
            ...
            {
              _id: "4",
              title: "Spring Cleaning vs. Deep Cleaning: What’s the...",
              slug: "spring",
              coverImage:
    ...
    <section className="py-24 bg-[#f2f2ee] font-poppins">
      <div className="max-w-[1320px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {/* Left Featured Post */}
          {posts[0] && (
            <article className="group flex flex-col h-[400px] md:h-[420px] justify-between">
              {/* Image + Tag */}
              <div className="relative overflow-hidden rounded-[40px] h-[70%]">
                <img
              ...
              {/* Content (metadata + title + button) */}
              <div className="flex flex-col justify-end h-[30%] mt-4">
                <div className="flex items-center gap-6 text-[#7a7a7a] text-[12px] font-semibold tracking-wider mb-3 uppercase">
                  <span className="inline-flex items-center gap-2">
                    <CalendarDays className="w-4 h-4 text-[#00A859]" />{" "}
                    {formatDate(posts[0].createdAt)}
                  </span>
                  <span className="inline-flex items-center gap-2">
                    <UserRound className="w-4 h-4 text-[#00A859]" /> BY{" "}
                    {posts[0].author || "ADMIN"}
                  </span>
                </div>
                <h3 className="text-[#1f2c3c] text-[18px] md:text-[20px] leading-tight font-bold mb-4 group-hover:text-[#00A859] transition-colors line-clamp-2">
                  {posts[0].title}
                </h3>
                <Link
                  to={`/blog/${posts[0].slug}`}
                  className="inline-flex items-center gap-3 text-[#1f2c3c] font-bold text-[18px] group-hover:text-[#00A859]"
                >
                  Read More
          ...
          <div className="flex flex-col gap-6 overflow-y-auto h-[400px] pr-4 scrollbar-hide">
            {posts.slice(1).map((post) => (
              <article
                key={post._id}
                className="group grid grid-cols-1 sm:grid-cols-[240px_1fr] gap-6 items-center border-b border-gray-200 pb-6 last:border-0 last:pb-0"
              >
                ...
                <div>
                  <div className="flex items-center gap-4 text-[#7a7a7a] text-[11px] font-semibold tracking-widest mb-2 uppercase">
                    <span className="inline-flex items-center gap-1.5">
                      <CalendarDays className="w-3.5 h-3.5" />{" "}
                      {formatDate(post.createdAt)}
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      <UserRound className="w-3.5 h-3.5" /> BY{" "}
                      {post.author || "ADMIN"}
                    </span>
                  </div>
                  <h3 className="text-[#1f2c3c] text-[22px] font-bold leading-tight mb-2 group-hover:text-[#00A859] transition-colors line-clamp-2">
                    {post.title}
                  </h3>
                  <Link
                    to={`/blog/${post.slug}`}
                    className="inline-flex items-center gap-2 text-[#1f2c3c] font-bold text-[15px] group-hover:text-[#00A859]"
                  >
                    Read More
